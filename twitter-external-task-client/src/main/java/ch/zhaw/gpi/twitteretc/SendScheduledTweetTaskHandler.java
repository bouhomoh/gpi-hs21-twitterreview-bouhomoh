...
public class SendScheduledTweetTaskHandler implements ExternalTaskHandler {
    ...
    public void execute(ExternalTask externalTask, ExternalTaskService externalTaskService) {
        ...
        System.out.println(new Date().toString() + ": Task Execution started for: " +
                externalTask.getProcessInstanceId() + instanceStarted);
        try {
            Thread.sleep(30000);
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
        System.out.println(new Date().toString() + ": Task Execution ended for: " +
                externalTask.getProcessInstanceId() + instanceStarted);
